...
const OurWork = () => {
    ...
    return (
        <div id="our-work" className="flex flex-col items-center gap-7 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white">
            <Title title='Our latest work' desc='Browse our portfolio of innovative digital products that showcase creativity, performance, and results.' />
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-5xl">
                {works.map((work, index) => (
                    <motion.div initial={{opacity: 0, y: 30}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.5, delay: index*0.2}} viewport={{once: true}} key={index} className="hover:scale-102 transition-transform duration-500 cursor-pointer">
